...
As established in [ADR 0001: Scanning Subsystem and Privilege Architecture](0001-scanning-and-privilege-architecture.md), scanning and analysis components remain strictly read-only. We now define the architecture and safety boundaries for guarded cleanup and storage mutation.
...
- **Immutable Exact Authorization**: An Action Plan represents an exact, immutable authorization identifying:
  ...
  - Strongest available Filesystem Object Object Identity.
...
- **Recycle Bin Operations**: Supported via platform IFileOperation with recycle intent and per-item result evaluation. Preflight evaluates volume support, media type, group policy, quota, and size thresholds where defensible without assuming fixed, removable, FAT, or exFAT volumes universally support or lack Recycle Bin. Explicit permanent deletion warnings from the platform are preserved and never auto-consented. When Windows provides item recovery evidence, PigTree records a Recovery Artifact with Windows as retention owner. If Windows warns of permanent deletion or fails to return recovery evidence, the operation stops and rejects rather than silently proceeding. Windows manages retention and restoration; PigTree records the reference and hands off. Immediate Reclaimable Allocation is reported as zero (or conditional upon eventual purge, subject to remaining Hard Links and open handles). Indefinite recovery is not guaranteed.
...
- **Journaled Sequence**:
  ...
  2. For recoverable mode: create and verify a preservation Hard Link to the victim Filesystem Object in a restricted per-volume PigTree recovery vault (for immediate-reclaim mode: create a temporary staging link). If staging or journal creation fails, the operation is ineligible.
...
- **Execution Records**: Every Action Execution emits an immutable Execution Record detailing source plan identity, snapshot interval, consent proofs, security context, live preconditions, attempt timestamps, Commit Points, per-step outcomes, errors, Recovery Artifacts, cancellation events, and post-verification observations. Records persist locally until explicitly deleted and are mandatory while Recovery Artifacts depend on them. Export is available only on explicit user request.
